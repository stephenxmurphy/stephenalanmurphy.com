...
s = s.replace('Design Manager, AD & NPD', 'Design Manager, Advanced and New Product Development')
...
""",
'company header cleanup')

old_early = """// Earlier work uses the same Company · Job Title band convention as the rest of the portfolio.
...
"""
new_early = """// Academic and internship projects keep Company · Job Title directly above each project title.
document.querySelectorAll('.early-company-band').forEach((band) => band.remove());
"""
replace_once(old_early, new_early, 'academic employer bands')

marker = "const projectDefinitions = ["
if marker not in s:
    raise SystemExit('Missing projectDefinitions marker')
professional = """const workSection = document.querySelector('#work');
...
"""
s = s.replace(marker, professional + marker, 1)
...
""",
'project shortcut blocks')

path.write_text(s)

Path('cleanup.css').write_text('''.site-header nav a[href^="mailto:"] {
  display: none;
}
